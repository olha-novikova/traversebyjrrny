...
function workscout_admin_scripts($hook){
	...
	if($hook=='edit-tags.php' || $hook == 'term.php'){
		...
		wp_enqueue_script( 'workscout-icon-selector', get_template_directory_uri() . '/js/iconselector.min.js', array('jquery'), '20150705', true );
	...
	if(!empty($api_key) && $geocode == 1){
		...
		wp_enqueue_script( 'workscout-wpjm-geo', get_template_directory_uri() . '/js/admin.workscout.maps.min.js', array('jquery'), '20150705', true );
	}
...
function workscout_scripts() {
	...
	wp_register_style( 'workscout-base', get_template_directory_uri(). '/css/base.min.css' );
    wp_register_style( 'workscout-responsive', get_template_directory_uri(). '/css/responsive.min.css' );
    wp_register_style( 'workscout-font-awesome', get_template_directory_uri(). '/css/font-awesome.min.css' );
	wp_enqueue_style( 'workscout-style', get_stylesheet_uri(), array('workscout-base','workscout-responsive','workscout-font-awesome') );
	wp_enqueue_style( 'workscout-woocommerce', get_template_directory_uri(). '/css/woocommerce.min.css' );
	...
	if ( defined( 'JOB_MANAGER_VERSION' ) ) {
	    ...
	    $wpdb->prepare('
	            ...
	            WHERE meta_key IN ("' . implode( '","', array( '_salary_min','_salary_max') ) . '")
		...
		$wpdb->prepare('
		    ...
		    FROM %1$s
		    LEFT JOIN %2$s ON %1$s.ID = %2$s.post_id
		    WHERE meta_key IN ("' . implode( '","', array(  '_salary_min','_salary_max' ) ) . '")  AND post_status = "publish"
		', $wpdb->posts, $wpdb->postmeta, '_salary_max' )
		) );
	    ...
	    $wpdb->prepare('
	            ...
	            FROM %1$s
	            LEFT JOIN %2$s ON %1$s.ID = %2$s.post_id
	            WHERE meta_key IN ("' . implode( '","',  array( '_rate_min') ) . '")
		...
		$wpdb->prepare('
		    ...
		    FROM %1$s
		    LEFT JOIN %2$s ON %1$s.ID = %2$s.post_id
		    WHERE meta_key IN ("' . implode( '","',  array( '_rate_max' ) ) . '")  AND post_status = "publish" AND post_type = "job_listing"
		', $wpdb->posts, $wpdb->postmeta, '_rate_max' )
		) );

		wp_dequeue_script('wp-job-manager-ajax-filters' );
		wp_deregister_script('wp-job-manager-ajax-filters');
		wp_register_script( 'workscout-wp-job-manager-ajax-filters', get_template_directory_uri() . '/js/workscout-ajax-filters.min.js', array( 'jquery', 'jquery-deserialize' ), '20150705', true );
	    ...
	    $wpdb->prepare('
	            ...
	            FROM %1$s
	            LEFT JOIN %2$s ON %1$s.ID = %2$s.post_id
	            WHERE  post_status = "publish" AND meta_key IN ("' . implode( '","',  array( '_rate_min') ) . '")
	            AND meta_value != ""
	        ', $wpdb->posts, $wpdb->postmeta )
	    ) );	
		...
		$wpdb->prepare('
		    ...
		    LEFT JOIN %2$s ON %1$s.ID = %2$s.post_id
		    WHERE post_status = "publish" AND meta_key IN ("' . implode( '","',  array( '_rate_min' ) ) . '") 
		', $wpdb->posts, $wpdb->postmeta, '_rate_min' )
		) );
		wp_dequeue_script('wp-resume-manager-ajax-filters' );
		wp_deregister_script('wp-resume-manager-ajax-filters');
		wp_register_script( 'workscout-wp-resume-manager-ajax-filters', get_template_directory_uri() . '/js/workscout-resumes-ajax-filters.min.js', array( 'jquery', 'jquery-deserialize' ), '20150705', true );
	...
	wp_enqueue_script( 'workscout-hoverIntent', get_template_directory_uri() . '/js/hoverIntent.min.js', array('jquery'), '20150705', true );
	...
	wp_enqueue_script( 'workscout-clusters', get_template_directory_uri() . '/js/markerclusterer.min.js', array('jquery'), '20150705', true );
	
	$map 		=  Kirki::get_option( 'workscout', 'pp_enable_jobs_map', 0 ); 
	$map_resume =  Kirki::get_option( 'workscout', 'pp_enable_resumes_map', 0 ); 
	if($map || $map_resume):
		wp_enqueue_script( 'workscout-map', get_template_directory_uri() . '/js/workscout.map.min.js', array('jquery'), '20150705', true );
	...
	wp_enqueue_script( 'workscout-single-map', get_template_directory_uri() . '/js/workscout.single.map.min.js', array('jquery'), '20150705', true );
	...
	wp_enqueue_script( 'workscout-jpanelmenu', get_template_directory_uri() . '/js/jquery.jpanelmenu.min.js', array('jquery'), '20150705', true );
	...
	wp_enqueue_script( 'workscout-superfish', get_template_directory_uri() . '/js/jquery.superfish.min.js', array('jquery'), '20150705', true );
	...
	wp_enqueue_script( 'workscout-stacktable', get_template_directory_uri() . '/js/stacktable.min.js', array('jquery'), '20150705', true );
	wp_enqueue_script( 'workscout-waypoints', get_template_directory_uri() . '/js/waypoints.min.js', array('jquery'), '20150705', true );
	wp_enqueue_script( 'workscout-headroom', get_template_directory_uri() . '/js/headroom.min.js', array('jquery'), '20150705', true );
	
	
	wp_enqueue_script( 'workscout-custom', get_template_directory_uri() . '/js/custom.min.js', array('jquery'), '20150705', true );
	$ajax_url = admin_url( 'admin-ajax.php', 'relative' );
